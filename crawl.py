...
while True:
    ...
    base64_str = re.search("base64,(.*?)'\)",response.text).group(1)  # 获取加密字符串，并转化为字典
    b = base64.b64decode(base64_str)                    #解析base64
    font = TTFont(io.BytesIO(b))                              #读取字体映射关系#用代码来获取编码和name的对应关系
    bestcmap = font['cmap'].getBestCmap()         #返回一个dict,其中key为int型编码,v为glyph名称
    newmap = dict()                                                  #把这个字典转一下，变成编码和正常字体的映射关系
    ...
    html = BeautifulSoup(response_,features="lxml")                    #lxml解析器
    ...
    for house in house_list:                                                             #截取住房网址，标题信息
        ...
        house_money=text                                                            #截取住房价格
        csv_writer.writerow([house_title, house_location, house_money, house_url])                 #写入csv文件
